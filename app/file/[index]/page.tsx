...
export default function FileDetailPage() {
  ...
  const analysis = store?.analyses[fileIndex] ?? null;
  const notFound = !isLoading && !analysis;
    ...
    return (
      <div className="min-h-screen bg-white dark:bg-slate-950 flex items-center justify-center">
        <div className="text-center space-y-4">
          <p className="text-slate-600 dark:text-slate-500 dark:text-slate-400 text-lg">File not found.</p>
    ...
    <div className="min-h-screen bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors">
      <header className="border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur sticky top-0 z-10 transition-colors">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center gap-4">
          <Link href="/" className="text-slate-600 dark:text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:text-slate-200 transition-colors flex items-center gap-1.5 text-sm">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        {/* Title */}
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100 font-mono break-all">{analysis.fileName}</h2>
          <p className="text-slate-600 dark:text-slate-500 text-sm mt-1">File index {fileIndex}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Status Codes */}
          <div>
            <h3 className="text-base font-semibold text-slate-800 dark:text-slate-200 mb-3">Status Codes</h3>
            <div className="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
              <table className="w-full border-collapse">
                <thead>
                  <tr>
                    <th className="py-2.5 px-4 text-left text-xs font-semibold text-slate-600 dark:text-slate-500 dark:text-slate-400 uppercase tracking-wider bg-slate-100 dark:bg-slate-900/60">Code</th>
                    <th className="py-2.5 px-4 text-right text-xs font-semibold text-slate-600 dark:text-slate-500 dark:text-slate-400 uppercase tracking-wider bg-slate-100 dark:bg-slate-900/60">Count</th>
                    <th className="py-2.5 px-4 text-right text-xs font-semibold text-slate-600 dark:text-slate-500 dark:text-slate-400 uppercase tracking-wider bg-slate-100 dark:bg-slate-900/60">%</th>
                  </tr>
                </thead>
                <tbody>
                  {statusBreakdown.map(({ code, count }) => (
                    <tr key={code} className="border-t border-slate-200 dark:border-slate-700/50 hover:bg-slate-50 dark:bg-slate-800/50 transition-colors">
                      ...
                      <td className="py-2.5 px-4 text-right font-mono text-sm text-slate-700 dark:text-slate-300">{count.toLocaleString()}</td>
                      <td className="py-2.5 px-4 text-right font-mono text-sm text-slate-600 dark:text-slate-500 dark:text-slate-400">
          ...
          {/* Content Types */}
          <div>
            <h3 className="text-base font-semibold text-slate-800 dark:text-slate-200 mb-3">Content Types</h3>
            <div className="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
              <table className="w-full border-collapse">
                <thead>
                  <tr>
                    <th className="py-2.5 px-4 text-left text-xs font-semibold text-slate-600 dark:text-slate-500 dark:text-slate-400 uppercase tracking-wider bg-slate-100 dark:bg-slate-900/60">Type</th>
                    <th className="py-2.5 px-4 text-right text-xs font-semibold text-slate-600 dark:text-slate-500 dark:text-slate-400 uppercase tracking-wider bg-slate-100 dark:bg-slate-900/60">Count</th>
                    <th className="py-2.5 px-4 text-right text-xs font-semibold text-slate-600 dark:text-slate-500 dark:text-slate-400 uppercase tracking-wider bg-slate-100 dark:bg-slate-900/60">%</th>
                  </tr>
                </thead>
                <tbody>
                  {contentTypeBreakdown.map(({ type, count }) => (
                    <tr key={type} className="border-t border-slate-200 dark:border-slate-700/50 hover:bg-slate-50 dark:bg-slate-800/50 transition-colors">
                      <td className="py-2.5 px-4">
                        <Link
        ...
        {perfStats && (
          <div>
            <h3 className="text-base font-semibold text-slate-800 dark:text-slate-200 mb-3">Performance Summary</h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
              {[
                ...
                <div key={label} className="bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl px-5 py-4">
                  <p className="text-xs text-slate-600 dark:text-slate-500 uppercase tracking-wider mb-1">{label}</p>
                  <p className="text-xl font-bold font-mono text-slate-900 dark:text-slate-100">{value}</p>
                  <p className="text-xs text-slate-600 dark:text-slate-500 dark:text-slate-600 mt-0.5">{sub}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Slowest */}
          <div>
            <h3 className="text-base font-semibold text-slate-800 dark:text-slate-200 mb-3">Slowest Requests <span className="text-sm font-normal text-slate-600 dark:text-slate-500">top 10</span></h3>
            <div className="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
              {slowest.length === 0 ? (
                <p className="text-slate-600 dark:text-slate-500 dark:text-slate-600 text-sm px-4 py-6 text-center">No data</p>
              ) : (() => {
                ...
                return (
                  <table className="w-full border-collapse">
                    <tbody>
                      {slowest.map((e, i) => (
                        <tr key={i} className="border-t border-slate-200 dark:border-slate-700/50 hover:bg-slate-50 dark:bg-slate-800/50 transition-colors">
                          <td className="py-2 px-3 text-xs font-mono text-slate-600 dark:text-slate-500 dark:text-slate-400 w-6 text-right">{i + 1}</td>
                          <td className="py-2 px-3 max-w-[180px]">
                            <Link
                              href={`/compare?url=${encodeURIComponent(e.url)}`}
                              className="text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:text-blue-300 hover:underline font-mono text-xs block truncate"
                              title={e.url}
                            >
                          ...
                          <td className="py-2 px-3 text-xs font-mono text-amber-600 dark:text-amber-400 text-right whitespace-nowrap">{formatTime(e.time)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {/* Largest */}
          <div>
            <h3 className="text-base font-semibold text-slate-800 dark:text-slate-200 mb-3">Largest Resources <span className="text-sm font-normal text-slate-600 dark:text-slate-500">top 10</span></h3>
            <div className="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
              {largest.length === 0 ? (
                <p className="text-slate-600 dark:text-slate-500 dark:text-slate-600 text-sm px-4 py-6 text-center">No data</p>
              ) : (() => {
                    ...
                    <tbody>
                      {largest.map((e, i) => (
                        <tr key={i} className="border-t border-slate-200 dark:border-slate-700/50 hover:bg-slate-50 dark:bg-slate-800/50 transition-colors">
                          ...
                          <td className="py-2 px-3 text-xs font-mono text-cyan-600 dark:text-cyan-400 text-right whitespace-nowrap">{formatBytes(e.contentSize)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        {timingAvgs && timingAvgs.total > 0 && (
          <div>
            <h3 className="text-base font-semibold text-slate-800 dark:text-slate-200 mb-3">
              ...
              <span className="ml-2 text-sm font-normal text-slate-600 dark:text-slate-500">per request phase</span>
            </h3>
            <div className="bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl p-5 space-y-4">
              ...
              <div>
                <div className="flex h-5 rounded-lg overflow-hidden gap-px">
                  {([
                    { key: 'dns',     label: 'DNS',     color: 'bg-blue-600 dark:bg-blue-500' },
                    { key: 'connect', label: 'Connect', color: 'bg-green-600 dark:bg-green-500' },
                    { key: 'ssl',     label: 'SSL',     color: 'bg-purple-600 dark:bg-purple-500' },
                    { key: 'send',    label: 'Send',    color: 'bg-slate-400' },
                    { key: 'wait',    label: 'TTFB',    color: 'bg-amber-600 dark:bg-amber-500' },
                    { key: 'receive', label: 'Receive', color: 'bg-cyan-600 dark:bg-cyan-500' },
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                {([
                  { key: 'dns',     label: 'DNS',     color: 'text-blue-600 dark:text-blue-400',   dot: 'bg-blue-600 dark:bg-blue-500'   },
                  { key: 'connect', label: 'Connect', color: 'text-green-600 dark:text-green-400',  dot: 'bg-green-600 dark:bg-green-500'  },
                  { key: 'ssl',     label: 'SSL',     color: 'text-purple-600 dark:text-purple-400', dot: 'bg-purple-600 dark:bg-purple-500' },
                  { key: 'send',    label: 'Send',    color: 'text-slate-700 dark:text-slate-300',  dot: 'bg-slate-400'  },
                  { key: 'wait',    label: 'TTFB',    color: 'text-amber-600 dark:text-amber-400',  dot: 'bg-amber-600 dark:bg-amber-500'  },
                  { key: 'receive', label: 'Receive', color: 'text-cyan-600 dark:text-cyan-400',   dot: 'bg-cyan-600 dark:bg-cyan-500'   },
              ...
              <p className="text-xs text-slate-600 dark:text-slate-500 dark:text-slate-600">Averages across {analysis.totalRequests.toLocaleString()} requests. TTFB = server think time (wait phase). Phases with &lt;0.5% share are hidden from bar.</p>
            </div>
          </div>
        )}
        ...
        <div>
          <div className="flex items-center justify-between mb-3 flex-wrap gap-3">
            <h3 className="text-base font-semibold text-slate-800 dark:text-slate-200">
              All Requests
              <span className="ml-2 text-sm font-normal text-slate-600 dark:text-slate-500">{filtered.length.toLocaleString()} entries</span>
            </h3>
            <input
              type="text"
              ...
              className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2 text-sm text-slate-800 dark:text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500 w-72"
            />
          </div>
