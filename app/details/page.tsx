...
function UrlGroupTable({ groups, analyses }: { groups: GroupedByUrl[]; analyses: HarAnalysis[] }) {
  ...
  if (groups.length === 0) {
    return <p className="text-center py-12 text-slate-600 dark:text-slate-500">No URLs found</p>;
  }
              ...
              return (
                <Fragment key={g.url}>
                  ...
                  >
                    ...
                    <td className="py-2.5 px-4 text-sm text-right font-mono text-slate-700 dark:text-slate-300">{formatTime(avgTime)}</td>
                  </tr>
                  {isExpanded && (
                            ...
                            <tbody>
                              {g.entries.map((e, i) => (
                                <tr key={i} className="border-t border-slate-200 dark:border-slate-700/30">
                                  ...
                                  <td className="py-1.5 pr-4 font-mono text-slate-700 dark:text-slate-300 text-right text-xs">{formatBytes(e.contentSize)}</td>
                                  <td className="py-1.5 font-mono text-slate-700 dark:text-slate-300 text-right text-xs">{formatTime(e.time)}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
