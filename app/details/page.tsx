      ...
      : type === "contentType"
        ...
        : type === "contentSizeBucket"
          ...
          : type === "serverIPAddress"
            ? value === "(no IP)"
              ? "Requests with No Server IP"
              : `Server IP: ${value}`
            : type === "userAgent"
              ? `User Agent: ${value}`
              : type === "method"
                ? value === "(no method)"
                  ? "Requests with No HTTP Method"
                  : `HTTP Method: ${value}`
                : "All URLs";
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-500 text-sm mt-1">
              {type === "url"
                ? groupedByUrl.length.toLocaleString() + " unique URLs"
                : filtered.length.toLocaleString() + " entries"}{" "}
              across {analyses.length} file{analyses.length !== 1 ? "s" : ""}
              {type === "userAgent" && value && (
                <span className="block text-xs text-slate-600 dark:text-slate-600 font-mono mt-1 break-all max-w-2xl">
                  {value}
                </span>
              )}
            </p>
          ...
          <input
            ...
            className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2 text-sm text-slate-800 dark:text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500 w-64"
          />
        </div>
                ...
                <thead>
                  <tr>
                    <th className={thClass} onClick={() => toggleSort("url")}>
                      URL{" "}
                      <SortIcon active={sortField === "url"} dir={sortDir} />
                    </th>
                    <th
                      className={thClass}
                ...
                <tbody>
                  {paginated.map((e, i) => (
                    ...
                    >
                      ...
                      <td
                        className="py-2.5 px-4 text-sm text-slate-600 dark:text-slate-400 font-mono text-xs max-w-[160px] truncate"
                        title={e.harFileName}
                      >
                        {e.harFileName}
                      </td>
...
function UrlGroupTable({
  ...
  if (groups.length === 0) {
    return (
      <p className="text-center py-12 text-slate-600 dark:text-slate-500">
        No URLs found
      </p>
    );
  }
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr>
              ...
              {analyses.map((a) => (
                <th
                  key={a.fileIndex}
                  className="py-3 px-4 text-right text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider bg-slate-100 dark:bg-slate-900/60 min-w-[120px]"
                >
                  <span
                    className="block truncate max-w-[130px] ml-auto"
                    title={a.fileName}
                  >
                    {a.fileName}
                  </span>
          ...
          <tbody>
            {groups.map((g) => {
              ...
              const avgSize =
                g.entries.reduce((s, e) => s + e.contentSize, 0) /
                g.entries.length;
              const avgTime =
                g.entries.reduce((s, e) => s + e.time, 0) / g.entries.length;

              return (
                <Fragment key={g.url}>
                  <tr
                    className="hover:bg-slate-50 dark:bg-slate-800/50 transition-colors border-t border-slate-200 dark:border-slate-700/50 cursor-pointer"
                    onClick={() => setExpandedUrl(isExpanded ? null : g.url)}
                  >
                    <td className="py-2.5 px-4 text-slate-600 dark:text-slate-500">
                      <span className="text-xs">{isExpanded ? "▼" : "▶"}</span>
                    </td>
                    <td
                      className="py-2.5 px-4 text-sm max-w-xs"
                    ...
                    <td className="py-2.5 px-4 text-sm text-right font-mono text-slate-700 dark:text-slate-300">
                      {g.entries.length}
                    </td>
                    {analyses.map((a) => (
                      <td
                        key={a.fileIndex}
                        className="py-2.5 px-4 text-sm text-right font-mono text-slate-700 dark:text-slate-300"
                      >
                        {g.byFile[a.fileIndex]?.length ?? (
                    ...
                    >
                      <td colSpan={5 + analyses.length} className="p-0">
                        <div className="bg-slate-100 dark:bg-slate-900/60 border-l-2 border-blue-500/40 px-6 py-4">
                          <p className="text-xs text-slate-600 dark:text-slate-400 font-mono break-all mb-3">
                            {g.url}
                          </p>
                          <table className="w-full text-sm border-collapse">
                            ...
                            <tbody>
                              {g.entries.map((e, i) => (
                                ...
                                >
                                  <td className="py-1.5 pr-4 font-mono text-xs text-slate-600 dark:text-slate-400 truncate max-w-[150px]">
                                    {e.harFileName}
                                  </td>
                                  <td
                                    className="py-1.5 pr-4 text-right text-xs font-mono text-slate-500"
                                    title={e.startedDateTime}
                                  >
                                  ...
                                  <td className="py-1.5 pr-4 text-right">
                                    <Link
                                      href={`/details?type=status&value=${e.status}`}
                                      onClick={(ev) => ev.stopPropagation()}
                                    >
                                      <StatusBadge code={e.status} />
                                    </Link>
                                  ...
                                  <td className="py-1.5 pr-4 text-right">
                                    <Link
                                      href={`/details?type=contentType&value=${encodeURIComponent(e.contentType)}`}
                                      className="text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:text-purple-300 text-xs font-mono"
                                      onClick={(ev) => ev.stopPropagation()}
                                    >
                                      {e.contentType}
                                    </Link>
                                  </td>
                                  <td className="py-1.5 pr-4 font-mono text-slate-700 dark:text-slate-300 text-right text-xs">
                                    {formatBytes(e.contentSize)}
                                  </td>
                                  <td className="py-1.5 font-mono text-slate-700 dark:text-slate-300 text-right text-xs">
                                    {formatTime(e.time)}
                                  </td>
                                </tr>
                              ))}
