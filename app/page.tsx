  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        <section>
          <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-4">Upload HAR Files</h2>
          ...
          <>
            <section>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-200">Loaded Files</h2>
                <span className="text-sm text-slate-600 dark:text-slate-500">{analyses.length} file{analyses.length !== 1 ? 's' : ''}</span>
              </div>
              <div className="flex flex-wrap gap-2">
                {analyses.map((a, i) => (
                  <div key={i} className="flex items-center gap-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm hover:border-blue-600 transition-colors">
                    <Link href={`/file/${i}`} className="flex items-center gap-2 min-w-0">
                      <span className="text-slate-700 dark:text-slate-300 font-mono truncate max-w-[200px]" title={a.fileName}>{a.fileName}</span>
                      <span className="text-slate-600 dark:text-slate-500 text-xs shrink-0">{a.totalRequests.toLocaleString()} reqs</span>
                    </Link>
                    <button
