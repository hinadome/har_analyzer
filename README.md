...
- **Content Diff page** — search by **pathname** (e.g. `/hello`). The banner shows **Selected path** and the entry list includes every host that requested that path (query strings ignored for matching, still shown on each row). Pick any two entries and view a line-by-line body diff with intra-line highlighting, JSON auto-prettification, unified and side-by-side modes, and an optional **Match full URL** escape hatch (exact URL including query). When either entry is binary (image, font, audio/video, octet-stream, zip, pdf) or has no captured body, the panel falls back to a SHA-256 hash comparison
- **Header Diff page** — same pathname search and entry selection as Content Diff, but diffs request headers, response headers, request cookies, and response cookies — added, removed, changed, and equal key-value pairs in a color-coded table
...
9. **Diff response bodies** — open **Content diff** from Tools (or `/content-diff`). Type a pathname such as `/hello` and select it. The banner shows **Selected path** `/hello`. The table lists every entry whose pathname is `/hello` on **any** host, regardless of query string, for example:
   ...
   Pick baseline and compare to diff the bodies. Enable **Match full URL** only when you need an exact URL **including** query.
10. **Diff headers and cookies** — open **Header diff** from Tools (or `/header-diff`); same pathname selection as content diff.
