...
- **Shared page shell** — `AppHeader`, `PageShell`, `EmptyState`, and `LoadingState` used across all analyzer routes; per-file color chips consolidated in `components/shared/fileColors.ts`.
...
- **`/anomalies`** — hub for pathname-level inconsistencies with four category pages (`/anomalies/status`, `/size`, `/encoding`, `/cache-policy`). Checks: mixed HTTP status on same path; response **size drift** (≥2× ratio or ≥10 KB delta among entries with `content.size` > 0); **Content-Encoding** drift or large uncompressed compressible responses (≥50 KB, no encoding); inconsistent **Cache-Control** or **Vary**. **Anomalies** in Tools with unique-path badge (or **clear**); insight strip and hub correlation strip when multiple checks hit the same path. Threshold constants in `utils/anomalies/analyze.ts`.
...
- **`DEPLOYMENT.md`** — 0.2.0 deploy guide: client-only architecture, pre-deploy `npm test`, worker chunks under `.next/static/`, `NEXT_PUBLIC_HAR_PARSE_WORKER` build arg, post-deploy smoke checklist (`/entry-diff`, `/mime-mismatch`, `/cache-validator`, `/anomalies`, Content-Type resolution).
- **`deploy-vm.sh`** — runs `npm test` before build; copies static assets to `.next/standalone/.next/static/` (matches Dockerfile); post-deploy smoke hints (entry-diff, mime-mismatch, cache-validator, anomalies, Content-Type split); honours `NEXT_PUBLIC_HAR_PARSE_WORKER` at build time when exported.
- **`Dockerfile` / `docker-compose.yml`** — optional `NEXT_PUBLIC_HAR_PARSE_WORKER` build arg.
- **`spec.md`** — synced for 0.2.0: home insight strip / Tools row, IndexedDB v2, privacy + worker flags, CORS inventory, kv-search pagination, `PageShell`, pathname-first `/entry-diff`, MIME mismatch, cache validator, anomalies hub, Content-Type resolution, `sample-hars/fixture-audits.har`.
