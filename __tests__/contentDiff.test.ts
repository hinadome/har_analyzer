...
describe("isBinaryEntry", () => {
  ...
  it.each([
    ...
    expect(
      isBinaryEntry(makeEntry({ contentType, responseContent: "data" })),
    ).toBe(true);
  });
  ...
  it.each([
    ...
    "returns false for %s with defined responseContent",
    (contentType, responseContent) => {
      expect(isBinaryEntry(makeEntry({ contentType, responseContent }))).toBe(
        false,
      );
    },
  );
...
describe("prettifyIfJson", () => {
  it("application/json with valid JSON → wasPrettified true and formatted text", () => {
    const { text, wasPrettified } = prettifyIfJson(
      '{"a":1}',
      "application/json",
    );
  ...
  it("content type ending in +json → wasPrettified true", () => {
    const { wasPrettified } = prettifyIfJson(
      '{"x":2}',
...
describe("Property 4 — Binary entry classification", () => {
  ...
  it("entries with non-binary contentType and defined responseContent → isBinaryEntry false", () => {
    const nonBinaryType = fc
      .string({ minLength: 1 })
      .filter((ct) => !BINARY_PREFIXES.some((p) => ct.startsWith(p)));
    fc.assert(
      fc.property(
        nonBinaryType,
        fc.string(),
  ...
  it("leftLines never contain added; rightLines never contain removed", () => {
    fc.assert(
      fc.property(fc.string(), fc.string(), (a, b) => {
        ...
        const rightHasRemoved = result.rightLines.some(
          (l) => l.kind === "removed",
        );
    ...
    fc.assert(
      fc.property(fc.string(), fc.string(), (a, b) => {
        ...
        const allPositive = (lines: typeof result.leftLines) =>
          nonPlaceholder(lines).every(
            (l) => l.lineNumber !== null && l.lineNumber > 0,
          );

        const strictlyIncreasing = (lines: typeof result.leftLines) => {
          const nums = nonPlaceholder(lines).map((l) => l.lineNumber as number);
    ...
    fc.assert(
      fc.property(singleLine, singleLine, (a, b) => {
        ...
        if (removedLine && removedLine.spans.length > 0) {
          if (
            removedLine.spans.map((s) => s.text).join("") !== removedLine.text
          )
            return false;
        }
        if (addedLine && addedLine.spans.length > 0) {
          if (addedLine.spans.map((s) => s.text).join("") !== addedLine.text)
            return false;
        }
        return true;
      }),
...
describe("Property 10 — Truncation", () => {
  ...
  it("body.length <= TRUNCATION_LIMIT, showFull false → text === body, wasTruncated false", () => {
    fc.assert(
      fc.property(fc.string({ maxLength: TRUNCATION_LIMIT }), (body) => {
        const { text, wasTruncated } = truncateBody(body, false);
        return text === body && wasTruncated === false;
      }),
    );
...
describe("sha256Hex", () => {
  // Known-good SHA-256 vectors (RFC / NIST examples)
  it("produces the canonical hash for an empty string", async () => {
    expect(await sha256Hex("")).toBe(
      "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855",
    );
  });

  it("produces the canonical hash for 'abc'", async () => {
  ...
  it("equal inputs produce equal hashes", async () => {
    const a = await sha256Hex("identical content");
    const b = await sha256Hex("identical content");
    expect(a).toBe(b);
  });
  ...
  it("handles large inputs without truncation differences", async () => {
    const big = "x".repeat(100_000);
    const bigPlus = big + "y";
    const a = await sha256Hex(big);
