...
if [[ "${1:-}" == "--update" ]]; then
  ...
  echo ""
  echo "  CORS:         /cors — Sec-Fetch-Mode gates ACAO; Cookie-only credentials; inventory when clean."
  echo "                Home insight strip shows CORS only on errors/warnings; Tools shows clear."
